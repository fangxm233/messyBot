...
import { ProcessStronghold4 } from "./instances/stronghold4";
...
export class Processes{
    ...
    static processStrongHold4(roomName: string, targetRoom: string): ProcessStronghold4{
        let process = new ProcessStronghold4(roomName, targetRoom);
        Process.startProcess(process);
        return process;
    }
    ...
    public static minePower(){
        for (const roomName in powerRegion) {
            ...
            if(storage && storage.energy < 660000) continue;
            let rooms = powerRegion[roomName];
            for (const powerRoomName of rooms) {
                if(!intel[powerRoomName]) continue;
                let i = intel[powerRoomName];
                if(i.powerBank && i.powerBank.ticks > 4000 && i.powerBank.amount > 4000 && Game.cpu.bucket > 7000){
    ...
    public static changeAttackRoom(attacker: string, from: string, to: string) {
        let flags = _.filter(Game.flags, flag => flag.pos.roomName == from && (flag.name.match('ar') || flag.name.match('ad')));
        let process: ProcessAttack = Process.getProcess(attacker, 'attack', 'targetRoom', from) as ProcessAttack;
        if(process) {
            process.targetRoom = to;
            process.memory.targetRoom = to;
        }
        flags.forEach(flag => flag.setPosition(new RoomPosition(flag.pos.x, flag.pos.y, to)));
    }

    public static interShardTransport(creepName?: string) {
    ...
    static rebuildProcess(processI: ProcessInterface, roomName: string, id: number){
        ...
        switch (processI.name) {
            ...
            case 'attackController':
                ...
                break;
            case 'stronghold4':
                process = ProcessStronghold4.getInstance(processI as ProcessStronghold4Interface, roomName);
                break;
            default:
                break;
        }
...
(global as any).Processes = Processes;