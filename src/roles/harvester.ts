        ...
        if(this.creep.pos.roomName == pos.roomName && Game.time % 10 == 0){
            ...
            if(enemies.length){
                ...
                if(!this.creep.room.memory.isClaimed) {
                    ...
                    if(!process) Porcesses.processDefend(this.creep.memory.spawnRoom, this.creep.room.name, 'creep');
                }
            }
            ...
            if(!this.creep.room.memory.isClaimed){
                ...
                if(cores.length){
                    let process = Process.getProcess(this.creep.memory.spawnRoom, 'defend', 'targetName', this.creep.room.name);
                    if(!process) Porcesses.processDefend(this.creep.memory.spawnRoom, this.creep.room.name, 'coreDis');
                }    
            }
        }
