...
@profile
export class RoleMelee extends Role{
    ...
    run() {
        ...
        if(!this.target) {
            ...
            let targets = room.find(FIND_HOSTILE_CREEPS, 
                { filter: creep => (RoleMelee.blackList[creep.name] || 0) < Game.time && creep.bodyCounts[WORK]});
            if(!targets.length) targets = room.find(FIND_HOSTILE_CREEPS, 
                { filter: creep => (RoleMelee.blackList[creep.name] || 0) < Game.time});
            if(!targets.length) return;
            this.target = targets[Math.floor(Math.random() * targets.length)].id;
        }

        let c = this.creep.pos.findClosestByRange(FIND_HOSTILE_CREEPS, {filter: c => c.pos.inRangeTo(this.creep, 1)});
        if(c) this.creep.attack(c);
        ...
        if(this.target) {
            ...
            if(this.checkWhenNextTick) {
                ...
                if(target.hits >= this.checkHit) {
                    RoleMelee.blackList[this.target] = Game.time + 10;
                    this.target = '' as any;
                    return;
                }
            }
            ...
            if(!this.creep.pos.inRangeTo(target, 1)) {
                if(this.timeOut >= 10) {
                    this.timeOut = 0;
                    this.target = '' as any;
                    RoleMelee.blackList[target.id] = Game.time + 10;
                }
                this.timeOut++;
            }
        }
