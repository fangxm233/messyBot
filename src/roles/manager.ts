...
import { ProcessBoost, compoundOrder } from "../process/instances/boost";
...
export class RoleManager extends Role {
            ...
            if (!order || order.amount == 0)
                ...
                if (tstore >= market.buyAmount) {
                    ...
                    if (need <= terminal.store.energy && terminal.cooldown == 0) {
                        ...
                        if (store.getUsedCapacity() - store.energy > 0) {
                            this.transferAll(terminal);
                            return;
                        }
    ...
    judgeState() {
        ...
        let processAttack = Process.getProcess(this.room.name, 'attack');
        if (processAttack) {
            let mappedLabs = _.map(labs, (lab, index) => ({ index, lab }));
            let unfullLabs = mappedLabs.filter(lab => {
                let compound = compoundOrder[lab.index];
                if (!this.terminal.store[compound]) return false;
                return 3000 - lab.lab.store[compound] < 1800
                // return !(lab.lab.mineralType == compound && (3000 - lab.lab.store[compound] > 1800) || this.terminal.store[compound] == 0);
            })
            if (unfullLabs.length) {
                this.setState('labC')
                return;
            }
        }
        ...
        if (Game.time % 2 == 0) resources = this.room.find(FIND_DROPPED_RESOURCES, { filter: t => t.resourceType != RESOURCE_ENERGY });
        if (resources.length || this.terminal.store.ops || this.storage.store.getUsedCapacity() - this.storage.store.energy - this.storage.store.getUsedCapacity(RESOURCE_OPS) > 0) {
        ...
        // if(!Memory.industry[roomName] && factory && (this.terminal.store.battery >= 50 && factory.store.battery < 10000 || factory.store.energy)) {
        //     this.setState('deBattery');
        //     return;
        // }
    ...
    depressBattery() {
        let factory = this.room.factory;
        if (!factory || !this.terminal || !this.storage) {
            this.clearState();
            return;
        }

        if (this.creep.store.battery) {
            this.transfer(factory, RESOURCE_BATTERY);
            return;
        }
        ...
        if (factory.store.energy >= 1500) {
            this.get(factory, RESOURCE_ENERGY);
            return;
        }
        if (factory.store.battery < 20000 && this.terminal.store.battery) {
            this.get(this.terminal, RESOURCE_BATTERY);
            return;
        }
    ...
    correct() {
        ...
        let resource = this.creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES, { filter: t => t.resourceType != RESOURCE_ENERGY });
        if (resource) {
            if (!this.creep.pos.isNearTo(resource)) this.creep.travelTo(resource);
            else this.creep.pickup(resource);
            return;
        }
        ...
        if (boostState == 'filling') {
            ...
            if (this.fillingState == 'fill') {
                for (let i = 0; i < processBoost.compoundTypes.length; i++) {
                    ...
                    const index = compoundOrder.findIndex(c => c == compound);
                    let demand = this.getBoostDemand(Game.creeps[processBoost.creepName], compound);
                    if (labs[index].store.energy < demand.energy && this.creep.store.energy) {
                        this.transfer(labs[index], RESOURCE_ENERGY, demand.energy - labs[index].energy);
                        return;
                    }
                    // if(labs[i].mineralType) continue;
                    if (!this.creep.store.getUsedCapacity(compound)) continue;
                    this.transfer(labs[index], compound);
                    return;
                }
    ...
    labC() {
        ...
        let unfullLabs = mappedLabs.filter(lab => {
            let compound = compoundOrder[lab.index];
            return !(lab.lab.mineralType == compound && lab.lab.store[compound] == lab.lab.store.getCapacity(compound) || this.terminal.store[compound] == 0 && this.creep.store[compound] == 0);
        })
        if (!unfullLabs.length) {
            this.clearState();
            return;
        ...
        if (this.creep.store.getUsedCapacity()) {
            let compound = Object.keys(this.creep.store)[0];
            let index = compoundOrder.findIndex(c => c == compound);
            let lab = unfullLabs.find(lab => lab.index == index);
            if (lab) {
                if (this.creep.pos.isNearTo(lab.lab)) this.creep.transfer(lab.lab, compound as any);
                else this.creep.travelTo(lab.lab);
                return;
            } else {
                this.setState('correct');
                return;
            ...
            for (let i = 0; i < unfullLabs.length; i++) {
                const lab = unfullLabs[i];
                let compound = compoundOrder[lab.index];
                if (this.terminal.store[compound]) {
                    this.get(this.terminal, compound as any, Math.min(lab.lab.mineralCapacity - lab.lab.mineralAmount, this.terminal.store[compound]));
                    return;
                }
            }
        }
    ...
    setState(state: 'none' | 'link' | 'ps' | 'deal' | 'nuker' | 'tower' | 'carry' | 'balance' | 'correct' | 'boost' | 'factory' | 'labC' | 'lab' | 'labE' | 'deBattery') {
    ...
    getDemandList(labs: StructureLab[], creep: Creep, compoundTypes: MineralBoostConstant[]): { [resource: string]: number } {
        ...
        for (let i = 0; i < compoundTypes.length; i++) {
            ...
            let mineralType = labs[index].mineralType;
            energy += creep.bodyCounts[part] * 20 - labs[index].store.energy - this.creep.store.energy;
            let remain = creep.bodyCounts[part] * 30 - (mineralType != null ? labs[index].store[mineralType] : 0) - this.creep.store.getUsedCapacity(compound);
    ...
    get(target: AnyStructure | Tombstone, type: ResourceConstant, amount?: number) {
        if (!target) return;
        if (this.creep.pos.getRangeTo(target) != 1) {
            this.creep.travelTo(target);
            return;
