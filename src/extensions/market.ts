...
const balanceTypes: ResourceConstant[] = [RESOURCE_ENERGY, RESOURCE_POWER, RESOURCE_OPS, RESOURCE_MIST];
...
export class Market {
    ...
    static run() {
        ...
        if (Memory.poorRoom) {
            for (const roomName of this.roomsToDeal) {
                ...
                if (!storage) continue;
                if (storage.store.energy > 750000 && !market.transport)
                    market.transport = { type: RESOURCE_ENERGY, amount: 100000, des: Memory.poorRoom };
            }
        }
        let o = this.getAllOrders({ resourceType: RESOURCE_ENERGY, type: ORDER_BUY });
        let mp = _.max(o, oo => oo.price);
    ...
    static handleExcess(sellEnergy: boolean) {
        for (const materialType of sellType) {
            ...
            if (materialType == RESOURCE_EMANATION) {
                let storedMoreThan = this.getStoredGreaterEqualThan(stored, materialType, 0, 'terminal');
    ...
    static changePrices() {
        ...
        for (const resourceType in minMarketPrices) {
            if (minMarketPrices.hasOwnProperty(resourceType)) {
                if (resourceType == RESOURCE_EMANATION) continue;
                let histories = Game.market.getHistory(resourceType as any);
                let history = histories[histories.length - 1];
                if (history) {
