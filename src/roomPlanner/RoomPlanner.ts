...
import { toRoomPosition } from '../utils';
...
export class RoomPlanner{
    ...
    public planRoom(targetName: string){
        ...
        for (const allot of Memory.rooms[targetName].allot[ALLOT_HARVEST]) {
            Memory.stableData[targetName].harvesterPosition[allot.id] = allot.data.pos;
        }
        ...
        for (const posId in stable.harvesterPosition) {
            const pos = stable.harvesterPosition[posId] as RoomPosition;
    ...
    public getProductLabs(): StructureLab[]{
        ...
        return _.compact([this.getForAtBase(STRUCTURE_LAB, labs[5]), this.getForAtBase(STRUCTURE_LAB, labs[6])]) as any;
    }
