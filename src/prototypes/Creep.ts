...
Object.defineProperty(Creep.prototype, 'boostedBodyCounts', {
	get() {
		if (!this._boostedBodyCounts) {
			this._boostedBodyCounts = _.countBy((this.body as BodyPartDefinition[]).filter(part => !!part.boost), bodyPart => bodyPart.type);
		}
		return this._boostedBodyCounts;
	},
	configurable: true,
});
