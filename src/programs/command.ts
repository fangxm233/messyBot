    ...
    static run() {
        ...
        for (const flagName in Game.flags) {
            if (Game.flags.hasOwnProperty(flagName)) {
                ...
                if (flag.name.match('hp')) { //支援房_hp_资源房_数量
                    let strings = flag.name.split('_');
                    let roomName = strings[0];
                    let sourceRoom = strings[2];
                    let creepNum = Number.parseInt(strings[3]);
                    if (!Process.getProcess(roomName, 'helping', 'targetRoom', flag.pos.roomName)) Processes.processHelping(roomName, flag.pos.roomName, sourceRoom, creepNum);
                    continue;
                }
