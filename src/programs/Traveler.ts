...
import { getRangeToEdge } from "../utils";
...
export class Traveler {
    ...
    public static travelTo(creep: Creep, destination: HasPos|RoomPosition, options: TravelToOptions = {}): number {
		...
		const distanceToEdge = getRangeToEdge(destination)
        ...
        if(travelData.path[0] !== undefined && options.pushCreep)
            this.pushCreeps(creep, Number.parseInt(travelData.path[0]));
    ...
    private static getPushDirection(pusher: Creep, pushee: Creep | PowerCreep){
        ...
        let dir = pusher.pos.getDirectionTo(pushee.pos);

        if(pusher.memory._trav != undefined && pusher.memory._trav.path[1] !== undefined){
            const nextDir = parseInt(pusher.memory._trav.path[1]);
        ...
        let matrix = ProcessActiveDefend.costMatrixs[pusher.room.name];
        if(matrix) _.remove(possiblePositions, pos => matrix.get(pos.x, pos.y) == DANGEROUS_ZONE);
        let swamps = _.remove(possiblePositions, pos => pos.lookFor(LOOK_TERRAIN)[0] == "swamp");
        ...
        //pushee.pos.getDirectionTo(possiblePositions[0]);
        else if(swamps.length) return pushee.pos.getDirectionTo(swamps[0]);
        else return pushee.pos.getDirectionTo(pusher.pos);
    }

    static getNowDir(creep: Creep): -1 | DirectionConstant{
        return _.get(creep, ['memory', '_trav', 'path', '0'], -1);
    }

    static getNextDir(creep: Creep): -1 | DirectionConstant{
        return _.get(creep, ['memory', '_trav', 'path', '1'], -1);
    }

    static comDirs(dir1: DirectionConstant, dir2: DirectionConstant): number{
        let d = Math.abs(dir2 - dir1);
        return d > 4 ? 8 - d : d;
    }
